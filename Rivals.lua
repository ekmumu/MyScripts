-- ==========================================
-- MUMU PRO (V74) - 不忘初心雷射神權版 (V60絕對防崩潰基底)
-- ==========================================
...
SendWebhookLog("💉 MUMU PRO [V74] 雷射神權版載入", "👤 **玩家:** " .. LocalPlayer.Name .. "\n🆔 **ID:** " .. LocalPlayer.UserId, 9214928)
...
local Settings = {
    ...
    -- 🚀 [神仙功能新增] 🚀
    SkinChanger = false, GodGun = false, Wallbang = false
}
...
-- 🚀 [絕對防崩潰：神仙功能背景引擎] 🚀
-- 這裡加入了嚴格的 pcall 防護，保證絕對不會再出現 attempt to call a nil value 的紅字！
task.spawn(function()
        ...
        for i = 1, #gc do
            ...
            if type(v) == "table" then
                ...
                if Settings.GodGun then
                    pcall(function()
                        if rawget(v, "FireRate") and rawget(v, "Ammo") then
                            v.FireRate = 0.001     -- 真正的雷射：每秒上百發
                            v.Ammo = 9999          -- 子彈無限
                            v.MaxAmmo = 9999
                            v.Spread = 0           -- 零擴散，形成一條線
                            v.MaxSpread = 0
                            v.Recoil = 0           -- 無後座力
...
CreateToggle(TabRage, "💎 全庫頂級解鎖 (Skin/Wrap)", "SkinChanger")
CreateToggle(TabRage, "🔫 終極雷射神槍 (極速+無後座)", "GodGun")
