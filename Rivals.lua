-- ==========================================
-- MUMU PRO (V28) - ESP抗崩潰 + 純扳機模式(TriggerBot)
-- ==========================================
...
local Settings = {
    ESP = true,
    Aimbot = true,        -- 實體鎖頭
    TriggerBot = false,   -- ⚡ 新增：純扳機 (準心指到就開火，不拉滑鼠)
...
if SafeGui:FindFirstChild("MUMU_PHYSICS_LOCK") then SafeGui.MUMU_PHYSICS_LOCK:Destroy() end

local ScreenGui = Instance.new("ScreenGui", SafeGui); ScreenGui.Name = "MUMU_PHYSICS_LOCK"; ScreenGui.ResetOnSpawn = false
local Main = Instance.new("Frame", ScreenGui); Main.Size = UDim2.fromOffset(330, 650); Main.Position = UDim2.fromScale(0.5, 0.5); Main.AnchorPoint = Vector2.new(0.5, 0.5); Main.BackgroundColor3 = Color3.fromRGB(15, 15, 15)
Instance.new("UICorner", Main).CornerRadius = UDim.new(0, 10)
local stroke = Instance.new("UIStroke", Main); stroke.Thickness = 3; stroke.Color = Color3.fromRGB(255, 0, 0)

local Title = Instance.new("TextLabel", Main); Title.Size = UDim2.new(1, 0, 0, 60); Title.Text = "⚡ MUMU PRO"; Title.TextSize = 28; Title.Font = Enum.Font.GothamBlack; Title.TextColor3 = Color3.new(1, 1, 1); Title.BackgroundTransparency = 1

local Container = Instance.new("ScrollingFrame", Main); Container.Size = UDim2.new(0.9, 0, 1, -100); Container.Position = UDim2.fromScale(0.05, 0.12); Container.BackgroundTransparency = 1; Container.ScrollBarThickness = 2; Container.CanvasSize = UDim2.new(0, 0, 2, 0) 
local Layout = Instance.new("UIListLayout", Container); Layout.Padding = UDim.new(0, 8)

local function AddToggle(name, settingKey)
    local btn = Instance.new("TextButton", Container); btn.Size = UDim2.new(1, -10, 0, 45); btn.BackgroundColor3 = Settings[settingKey] and Color3.fromRGB(200, 0, 0) or Color3.fromRGB(45, 45, 45); btn.Text = name .. ": " .. (Settings[settingKey] and "ON" or "OFF"); btn.TextSize = 16; btn.Font = Enum.Font.GothamBold; btn.TextColor3 = Color3.new(1, 1, 1); Instance.new("UICorner", btn)
    btn.MouseButton1Click:Connect(function() Settings[settingKey] = not Settings[settingKey]; btn.Text = name .. ": " .. (Settings[settingKey] and "ON" or "OFF"); btn.BackgroundColor3 = Settings[settingKey] and Color3.fromRGB(200, 0, 0) or Color3.fromRGB(45, 45, 45) end)
end

local function AddAdjuster(name, settingKey, step, min, max)
    local frame = Instance.new("Frame", Container); frame.Size = UDim2.new(1, -10, 0, 45); frame.BackgroundColor3 = Color3.fromRGB(35, 35, 35); Instance.new("UICorner", frame)
    local title = Instance.new("TextLabel", frame); title.Size = UDim2.new(0.55, 0, 1, 0); title.Position = UDim2.new(0.05, 0, 0, 0); title.Text = name; title.TextColor3 = Color3.new(1, 1, 1); title.Font = Enum.Font.GothamBold; title.TextSize = 14; title.TextXAlignment = Enum.TextXAlignment.Left; title.BackgroundTransparency = 1
    local valText = Instance.new("TextLabel", frame); valText.Size = UDim2.new(0.2, 0, 1, 0); valText.Position = UDim2.new(0.65, 0, 0, 0); valText.Text = tostring(Settings[settingKey]); valText.TextColor3 = Color3.new(1, 1, 1); valText.Font = Enum.Font.GothamBold; valText.TextSize = 15; valText.BackgroundTransparency = 1
    local btnMinus = Instance.new("TextButton", frame); btnMinus.Size = UDim2.new(0, 30, 0, 30); btnMinus.Position = UDim2.new(0.55, 0, 0.5, -15); btnMinus.BackgroundColor3 = Color3.fromRGB(80, 20, 20); btnMinus.Text = "-"; btnMinus.TextColor3 = Color3.new(1, 1, 1); btnMinus.Font = Enum.Font.GothamBold; btnMinus.TextSize = 20; Instance.new("UICorner", btnMinus)
    local btnPlus = Instance.new("TextButton", frame); btnPlus.Size = UDim2.new(0, 30, 0, 30); btnPlus.Position = UDim2.new(0.85, 0, 0.5, -15); btnPlus.BackgroundColor3 = Color3.fromRGB(20, 80, 20); btnPlus.Text = "+"; btnPlus.TextColor3 = Color3.new(1, 1, 1); btnPlus.Font = Enum.Font.GothamBold; btnPlus.TextSize = 20; Instance.new("UICorner", btnPlus)
    btnMinus.MouseButton1Click:Connect(function() if Settings[settingKey] - step >= min then Settings[settingKey] = Settings[settingKey] - step; valText.Text = tostring(Settings[settingKey]) end end)
    btnPlus.MouseButton1Click:Connect(function() if Settings[settingKey] + step <= max then Settings[settingKey] = Settings[settingKey] + step; valText.Text = tostring(Settings[settingKey]) end end)
end
...
local function UpdateFlyState(state)
    ...
    if state then
        if not hrp:FindFirstChild("MUMU_GYRO") then FlyBodyGyro = Instance.new("BodyGyro", hrp); FlyBodyGyro.Name = "MUMU_GYRO"; FlyBodyGyro.P = 9e4; FlyBodyGyro.maxTorque = Vector3.new(9e9, 9e9, 9e9); FlyBodyGyro.cframe = hrp.CFrame end
        if not hrp:FindFirstChild("MUMU_VELOCITY") then FlyBodyVelocity = Instance.new("BodyVelocity", hrp); FlyBodyVelocity.Name = "MUMU_VELOCITY"; FlyBodyVelocity.velocity = Vector3.new(0, 0, 0); FlyBodyVelocity.maxForce = Vector3.new(9e9, 9e9, 9e9) end
...
AddToggle("純扳機 (指到敵人自動開火)", "TriggerBot") -- ⚡ 新增：不鎖頭自動開火
...
local Hint = Instance.new("TextLabel", Main); Hint.Size = UDim2.new(1, 0, 0, 25); Hint.Position = UDim2.new(0, 0, 1, -25); Hint.Text = "按 [J] 顯示/隱藏面板"; Hint.TextColor3 = Color3.fromRGB(150, 150, 150); Hint.TextSize = 13; Hint.Font = Enum.Font.Gotham; Hint.BackgroundTransparency = 1

UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if not gameProcessed and input.KeyCode == Enum.KeyCode.J then Main.Visible = not Main.Visible end
    if not gameProcessed then
        local k = input.KeyCode; if k == Enum.KeyCode.W then CONTROL.F = 1 elseif k == Enum.KeyCode.S then CONTROL.B = -1 elseif k == Enum.KeyCode.A then CONTROL.L = -1 elseif k == Enum.KeyCode.D then CONTROL.R = 1 elseif k == Enum.KeyCode.Space then CONTROL.UP = 1 elseif k == Enum.KeyCode.LeftControl then CONTROL.DOWN = -1 end
    end
end)

UserInputService.InputEnded:Connect(function(input, gameProcessed)
    local k = input.KeyCode; if k == Enum.KeyCode.W then CONTROL.F = 0 elseif k == Enum.KeyCode.S then CONTROL.B = 0 elseif k == Enum.KeyCode.A then CONTROL.L = 0 elseif k == Enum.KeyCode.D then CONTROL.R = 0 elseif k == Enum.KeyCode.Space then CONTROL.UP = 0 elseif k == Enum.KeyCode.LeftControl then CONTROL.DOWN = 0 end
end)
                ...
                if Settings.ESP and p.Character and p.Character:FindFirstChild("HumanoidRootPart") and p.Character:FindFirstChild("Humanoid") and p.Character.Humanoid.Health > 0 then
                    ...
                    if dist < Settings.MaxDistance then
                        ...
                        if onScreen then
                            ...
                            if maxHealth <= 0 then maxHealth = 100 end -- 強制修正為 100 防除以零
                            
                            local healthPercent = math.clamp(health / maxHealth, 0, 1)
                            if healthPercent ~= healthPercent then healthPercent = 1 end -- 防止 NaN
...
_G.MUMU_PRO_CONNECTION = RunService.RenderStepped:Connect(function()
    ...
    if Settings.Fly and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
        ...
        if gyro and vel then
            local camCF = Camera.CFrame; gyro.cframe = camCF; local moveDir = Vector3.new(0, 0, 0)
    ...
    if Settings.Aimbot or Settings.AutoFire_Hip or Settings.AutoFire_ADS or Settings.TriggerBot then
        ...
        if LockedTarget and LockedTarget:FindFirstChild("Head") and LockedTarget:FindFirstChild("Humanoid") and LockedTarget.Humanoid.Health > 0 then
            ...
            if onScreen then
                ...
                -- 模式 B: 盲射鎖頭開火
                if Settings.AutoFire_Hip and not isRightClicking then shouldFire = true end
